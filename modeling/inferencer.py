...
class LinearCRF(nn.Module):
    ...
    def __init__(self, label2idx: Dict[str, int], idx2labels: List[str] = None, START_TAG='START', STOP_TAG='STOP', PAD_TAG='PAD', iobes=False):
        ...
        self.pad_idx = self.label2idx[PAD_TAG]
        ...
        init_transition[self.pad_idx, :] = -10000.0
        if iobes:
            for prev_label, next_label in itertools.product(label2idx, label2idx):
                if prev_label == "O" and (next_label.startswith("I") or next_label.startswith("E")):
                    init_transition[self.label2idx[prev_label], self.label2idx[next_label]] = -10000.0
